...
import TextRepeaterView from '@/views/text-tools/TextRepeaterView';
...
const SEO_CONFIG: ToolPageSeoConfig = {
  ...
  namespace: 'text-tools/TextRepeaterTool',
};

interface TextRepeaterToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: TextRepeaterToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function TextRepeaterToolPage({
  params,
}: TextRepeaterToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <TextRepeaterView lang={lang as Language} />
    </>
  );
}
