...
interface RemoveFormattingToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: RemoveFormattingToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function RemoveFormattingToolPage({
  params,
}: RemoveFormattingToolPageProps) {
  const { lang } = await params;
