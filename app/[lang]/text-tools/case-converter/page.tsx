...
import CaseConverterView from '@/views/text-tools/CaseConverterView';
...
const SEO_CONFIG: ToolPageSeoConfig = {
  ...
  namespace: 'text-tools/CaseConverterTool',
};

interface CaseConverterToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: CaseConverterToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function CaseConverterToolPage({
  params,
}: CaseConverterToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <CaseConverterView lang={lang as Language} />
    </>
  );
}
