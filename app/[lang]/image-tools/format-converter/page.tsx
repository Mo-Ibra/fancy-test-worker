...
import FormatConvertView from '@/views/image-tools/FormatConvertView';
import ToolJsonLd from '@/components/seo/ToolJsonLd';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

const SEO_CONFIG: ToolPageSeoConfig = {
  path: '/image-tools/format-converter',
  categoryPath: '/image-tools',
  namespace: 'image-tools/FormatConvertTool',
};

interface FormatConvertToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: FormatConvertToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function FormatConvertToolPage({
  params,
}: FormatConvertToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <FormatConvertView lang={lang as Language} />
    </>
  );
}
