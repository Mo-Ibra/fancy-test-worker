...
import GrayscaleConverterView from '@/views/image-tools/GrayscaleConverterView';
import ToolJsonLd from '@/components/seo/ToolJsonLd';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

const SEO_CONFIG: ToolPageSeoConfig = {
  path: '/image-tools/grayscale-converter',
  categoryPath: '/image-tools',
  namespace: 'image-tools/GrayscaleConverterTool',
};

interface GrayscaleConverterToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: GrayscaleConverterToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function GrayscaleConverterToolPage({
  params,
}: GrayscaleConverterToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <GrayscaleConverterView lang={lang as Language} />
    </>
  );
}
