...
import AddWatermarkView from '@/views/image-tools/AddWatermarkView';
...
const SEO_CONFIG: ToolPageSeoConfig = {
  ...
  namespace: 'image-tools/AddWatermarkTool',
};

interface AddWatermarkToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: AddWatermarkToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function AddWatermarkToolPage({
  params,
}: AddWatermarkToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <AddWatermarkView lang={lang as Language} />
    </>
  );
}
