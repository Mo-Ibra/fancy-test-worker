...
import ImageCompressorView from '@/views/image-tools/ImageCompressorView';
...
const SEO_CONFIG: ToolPageSeoConfig = {
  ...
  namespace: 'image-tools/ImageCompressorTool',
};

interface ImageCompressorToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: ImageCompressorToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function ImageCompressorToolPage({
  params,
}: ImageCompressorToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <ImageCompressorView lang={lang as Language} />
    </>
  );
}
