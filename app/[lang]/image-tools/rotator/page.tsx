...
import ImageRotatorView from '@/views/image-tools/ImageRotatorView';
import ToolJsonLd from '@/components/seo/ToolJsonLd';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

const SEO_CONFIG: ToolPageSeoConfig = {
  path: '/image-tools/rotator',
  categoryPath: '/image-tools',
  namespace: 'image-tools/ImageRotatorTool',
};

interface ImageRotatorToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: ImageRotatorToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function ImageRotatorToolPage({
  params,
}: ImageRotatorToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <ImageRotatorView lang={lang as Language} />
    </>
  );
}
