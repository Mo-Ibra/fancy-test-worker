...
import UnixTimestampView from '@/views/dev-tools/UnixTimestampView';
import ToolJsonLd from '@/components/seo/ToolJsonLd';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

const SEO_CONFIG: ToolPageSeoConfig = {
  path: '/dev-tools/unix-timestamp',
  categoryPath: '/dev-tools',
  namespace: 'dev-tools/UnixTimestampTool',
};

interface UnixTimestampToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: UnixTimestampToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function UnixTimestampToolPage({
  params,
}: UnixTimestampToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <UnixTimestampView lang={lang as Language} />
    </>
  );
}
