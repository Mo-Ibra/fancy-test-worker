...
import JWTDecoderView from '@/views/dev-tools/JWTDecoderView';
...
const SEO_CONFIG: ToolPageSeoConfig = {
  ...
  namespace: 'dev-tools/JWTDecoderTool',
};

interface JWTDecoderToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: JWTDecoderToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function JWTDecoderToolPage({
  params,
}: JWTDecoderToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <JWTDecoderView lang={lang as Language} />
    </>
  );
}
