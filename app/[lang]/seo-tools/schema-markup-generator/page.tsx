...
import SchemaMarkupGeneratorView from '@/views/seo-tools/SchemaMarkupGeneratorView';
import ToolJsonLd from '@/components/seo/ToolJsonLd';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

const SEO_CONFIG: ToolPageSeoConfig = {
  path: '/seo-tools/schema-markup-generator',
  categoryPath: '/seo-tools',
  namespace: 'seo-tools/SchemaMarkupGeneratorTool',
};

interface SchemaMarkupGeneratorToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: SchemaMarkupGeneratorToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function SchemaMarkupGeneratorToolPage({
  params,
}: SchemaMarkupGeneratorToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <SchemaMarkupGeneratorView lang={lang as Language} />
    </>
  );
}
