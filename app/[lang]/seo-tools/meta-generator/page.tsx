...
import MetaTagGeneratorView from '@/views/seo-tools/MetaTagGeneratorView';
...
const SEO_CONFIG: ToolPageSeoConfig = {
  ...
  namespace: 'seo-tools/MetaTagGeneratorTool',
};

interface MetaTagGeneratorToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: MetaTagGeneratorToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function MetaTagGeneratorToolPage({
  params,
}: MetaTagGeneratorToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <MetaTagGeneratorView lang={lang as Language} />
    </>
  );
}
