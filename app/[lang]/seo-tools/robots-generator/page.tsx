...
import RobotsTxtGeneratorView from '@/views/seo-tools/RobotsTxtGeneratorView';
import ToolJsonLd from '@/components/seo/ToolJsonLd';
import { Metadata } from 'next';
import { notFound } from 'next/navigation';

const SEO_CONFIG: ToolPageSeoConfig = {
  path: '/seo-tools/robots-generator',
  categoryPath: '/seo-tools',
  namespace: 'seo-tools/RobotsTxtGeneratorTool',
};

interface RobotsTxtGeneratorToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: RobotsTxtGeneratorToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function RobotsTxtGeneratorToolPage({
  params,
}: RobotsTxtGeneratorToolPageProps) {
  const { lang } = await params;
  ...
  return (
    <>
      ...
      <RobotsTxtGeneratorView lang={lang as Language} />
    </>
  );
}
