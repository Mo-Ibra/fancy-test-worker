...
interface CanonicalTagGeneratorToolPageProps {
  params: Promise<{ lang: string }>;
}

export async function generateMetadata({
  params,
}: CanonicalTagGeneratorToolPageProps): Promise<Metadata> {
  const { lang } = await params;
  return generateToolPageMetadata(lang as Language, SEO_CONFIG);
}

export default async function CanonicalTagGeneratorToolPage({
  params,
}: CanonicalTagGeneratorToolPageProps) {
  const { lang } = await params;
