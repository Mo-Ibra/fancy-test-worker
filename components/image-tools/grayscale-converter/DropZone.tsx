...
export default function DropZone({ onFile }: { onFile: (f: File) => void }) {
  ...
  return (
    <div
      ...
      className={`group flex flex-col items-center justify-center gap-4 h-64 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 ${drag ? "border-blue-400 bg-blue-50 dark:bg-blue-900/20"
    ...
    >
      ...
      <div className="text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">{t("dropzone.formats")}</p>
      </div>
    </div>
  );
}