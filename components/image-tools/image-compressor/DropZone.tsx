...
export default function DropZone({ onFiles }: { onFiles: (f: File[]) => void }) {
  ...
  return (
    <div
      ...
      className={`group flex flex-col items-center justify-center gap-4 h-52 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-200 ${drag ? "border-blue-400 bg-blue-50 dark:bg-blue-900/20" : "border-border hover:border-blue-300 dark:hover:border-blue-700 bg-card hover:bg-blue-50/40 dark:hover:bg-blue-900/10"
        }`}
    >
      ...
      <div className="text-center px-4">
        ...
        <p className="text-xs text-muted-foreground mt-1">{t("dropzone.formatsSupported")}</p>
      </div>
    </div>
  );
}